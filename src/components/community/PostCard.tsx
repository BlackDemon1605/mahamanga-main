...
import { Heart, MessageCircle, Trash2, Send, ChevronDown, ChevronUp, Pin } from 'lucide-react';
...
export function PostCard({ post }: { post: Post }) {
  ...
  return (
    <div className={`glass rounded-xl p-4 animate-fade-up ${post.is_pinned ? 'border border-primary/40 relative' : ''}`}>
      {post.is_pinned && (
            ...
            <div className="flex items-center gap-1">
              ...
              {isAdmin && (
                <Button
                  ...
                  className={`${post.is_pinned ? 'text-primary' : 'text-muted-foreground'} hover:text-primary`}
                  onClick={() => togglePin.mutate()}
                  title={post.is_pinned ? 'Unpin post' : 'Pin post'}
                >
                  <Pin className={`w-4 h-4 ${post.is_pinned ? 'fill-primary' : ''}`} />
                </Button>
              )}
