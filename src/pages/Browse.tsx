...
const STANDARD_GENRES = [
  'Action', 'Adventure', 'Comedy', 'Drama', 'Fantasy', 'Horror', 'Romance', 'Sci-Fi', 'Slice of Life', 'Sports',
];

const GENRE_TABS = [
  { value: 'all', label: 'All' },
  ...STANDARD_GENRES.map(g => ({ value: g, label: g })),
...
export default function Browse() {
  ...
  const { data: comics, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (genre === 'Other' && data) {
        const standardLower = STANDARD_GENRES.map(g => g.toLowerCase());
        return data.filter(comic => 
          comic.genre?.some((g: string) => !standardLower.includes(g.toLowerCase()))
        );
      }
