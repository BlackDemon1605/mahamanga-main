...
import { Shield, Image, Plus, Trash2, Users, BookOpen, BarChart3, Upload, LogIn, Pencil, Megaphone } from 'lucide-react';
    ...
    <MainLayout>
      <div className="px-4 py-6 max-w-7xl mx-auto">
        ...
        <Tabs defaultValue={isAdmin ? "banners" : "comics"} className="w-full">
          <TabsList className={`w-full grid ${isAdmin ? 'grid-cols-4' : 'grid-cols-1'} bg-secondary rounded-xl p-1 mb-6`}>
            {isCreator && (
          ...
          {isAdmin && (
            <TabsContent value="broadcast">
              <AdminBroadcast profileId={profile?.id || ''} />
            </TabsContent>
          )}
...
// Admin Broadcast sub-component
function AdminBroadcast({ profileId }: { profileId: string }) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // Insert a notification for each user (batch)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="space-y-2">
          <Label>Title</Label>
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="📢 Community update from MahaManga!" maxLength={120} />
        </div>
        <div className="space-y-2">
          <Label>Message (optional)</Label>
          <Textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Details about the update..." rows={3} maxLength={500} />
        </div>
        <Button variant="gradient" onClick={handleSend} disabled={!title.trim() || sending} className="w-full">
          <Megaphone className="w-4 h-4 mr-2" />
